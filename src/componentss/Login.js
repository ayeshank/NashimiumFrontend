...
export default function Login() {
  ...
  const postData=async (e)=>{
   ...
   if(res.status == 400) {  alert("Member not found"); }
   else if(res.status == 401) {  alert("Password is Invalid"); }
...
<Form.Control type="text" name="memberno" placeholder="100245678231" value={member.memberno} onChange={handleInputs} required/>
...
<Button variant="success" style={{width:"100%"}} type="submit" onClick={postData}>{t('Login')}</Button>

<Form.Text className="text-muted"><Link to="/forgotpassword">{t('Forgot Password')}</Link></Form.Text>
<hr/>
<Button variant="secondary" onClick={()=>navigate('/otp')} style={{width:"100%"}} >{t('Create an Account')}</Button>
</Form>
</div>
  )
}
