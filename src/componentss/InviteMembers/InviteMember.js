...
export default function InviteMember() {
  ...
  const postData=async (e)=>{
    ...
    alert(code);
   const res=await fetch("/InviteMemberReq",{
     method:"POST",
     headers:{
   ...
   if(res.status === 500 || !data) {  window.alert("Fail to send email to invited member"); }
   else{ 
     sendEmail();
    // alert("Email sent");
    }
...
if(!memberData)
{
  ...
  return (
    ...
    <h6 className="xnarateh6" style={{backgroundColor:"rgb(8, 143, 114)"}}>{t('Invite Members')}</h6>
...
/>
    ...
    <Form.Label htmlFor="basic-url">{t('Enter Email Address of Invited Member')}</Form.Label>
  <InputGroup className="mb-3">
<FormControl
  placeholder={t('Enter Email Address')}
  aria-describedby="basic-addon2"
  type="email"
  name="rec" value={newMemberEmail} onChange={e=>setnewMemberEmail(e.target.value)}
/>
