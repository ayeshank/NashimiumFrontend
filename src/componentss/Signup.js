  ...
  const postData = async (e) => {
    ...
    if (parseFloat(payAud) < parseFloat(data.NewMembershipInvest)) {
      ...
      const res = await fetch(
        "https://nashimiumbackend.herokuapp.com/membersignup",
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    async function fetchBooks() {
      const response = await fetch(
        "https://nashimiumbackend.herokuapp.com/membersfeelastget"
      );
      const json = await response.json();
      setData(json.ml);
      const response2 = await fetch(
        "https://nashimiumbackend.herokuapp.com/memberlastget"
      );
