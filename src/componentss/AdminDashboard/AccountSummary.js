...
export default function AccountSummary() {
    ...
    const [compsetbal, setCOMPsetbal] = useState({});
      ...
      useEffect(() => {
        ...
        { const response = await fetch('/xnalastget'); const json = await response.json(); setXNAbal(json.xl); }
        async function fetchaud()
        { const response = await fetch('/audlastget'); const json = await response.json(); setAUDbal(json.al); }
        async function fetchcomp()
        { const response = await fetch('/companylastget'); const json = await response.json(); setCOMPbal(json.cl); }
        async function fetchcompset()
        { const response = await fetch('/complastsetget'); const json = await response.json(); setCOMPsetbal(json.csl); }
        fetchxna();fetchaud();fetchcomp();fetchcompset();
  },[]);
  if (!compsetbal.length || !xnabal.length || !audbal.length || !compbal.length) {
    return null;
  }
  return (
  ...
  <tr>
        ...
        <tr>
        <td>Total AUD</td>
            <td>$ {(parseFloat(compbal[0].CB)+parseFloat(audbal[0].audnet)).toFixed(2)}</td>
        </tr>
        <tr>
           <td>XNA/AUD</td>
        ...
        <tr>
           <td>Buy Rate</td>
          <td>{(parseFloat(audbal[0].xnaaud)*(1+parseFloat(compsetbal[0].ExRate))).toFixed(10)}</td>
        </tr>
        <tr>
           <td>Sell Rate</td>
          <td>{(parseFloat(audbal[0].xnaaud)/(1+parseFloat(compsetbal[0].ExRate))).toFixed(10)}</td>
