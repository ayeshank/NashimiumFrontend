...
export default function MemberInformation() {
    ...
    const [member,setMember]=useState({adminname:"",adminno:"",AUD:"",CAD:"",CHF:"",EUR:"",GBP:"",KWD:"",TRY:"",USD:"",XNA:"",NewMembershipInvest:"",MonthlyMembershipFee:""})
    ...
    async function fetchBooks() {
      ...
      setMember(json.mf2[0]);
        ...
        const postData=async (e)=>{
          ...
          const res=await fetch("/memfee",{
            method:"POST",
            headers:{
              "Content-Type":"application/json"
            },
            body:JSON.stringify({
              AUD,NewMembershipInvest,MonthlyMembershipFee
            })
          });
          const data=await res.json();
          if(res.status === 500 || !data) {  window.alert("Failed To Updated Membership Fee Setting"); }
          else{ window.alert("Updated Membership Fee Setting");
       } }
    ...
    {show==true?
      <Modal show={show}>
            ...
            <Modal.Body>
              <Form >
              <Row >
                  <Col ><Form.Text className="text-muted">{t('Membership Amount In AUD')}</Form.Text></Col>
                  <Col><Form.Control size="sm" type="text" name="AUD" placeholder="Enter Membership Amount In AUD"  value={member.AUD}  onChange={e=>handleInputs(e)}/></Col>
              </Row>
              {/* <Row >
              ...
              <Row >
                  <Col ><Form.Text className="text-muted">{t('New Membership Investment In AUD')}</Form.Text></Col>
                  <Col><Form.Control size="sm" type="text" name="NewMembershipInvest" placeholder="" value={member.NewMembershipInvest} onChange={e=>handleInputs(e)}/></Col>
              </Row>
              <Row >
                  <Col ><Form.Text className="text-muted">{t('Monthly Membership Fee In AUD')}</Form.Text></Col>
                  <Col><Form.Control size="sm" type="text" name="MonthlyMembershipFee" placeholder="" value={member.MonthlyMembershipFee} onChange={e=>handleInputs(e)}/></Col>
              </Row>
     </Form>
            </Modal.Body>
            <Modal.Footer>
