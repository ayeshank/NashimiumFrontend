...
export default function SellAmount() {
  ...
  const postData=async (e)=>{
    e.preventDefault();
    if (parseFloat(sellxna) > parseFloat((Math.floor((parseFloat(totdeposit)-parseFloat(totwithdraw))*(parseFloat(databs[0].SLP)/100))*1000000000/1000000000).toFixed(10)))
    {
      console.log(parseFloat(sellxna))
      console.log((Math.floor((parseFloat(totdeposit)-parseFloat(totwithdraw))*(parseFloat(databs[0].SLP)/100))*1000000000/1000000000).toFixed(10))
      alert("Sell Limit exceeded")
    }
    else if   (parseFloat(sellxna) < (Math.floor( parseFloat(databs[0].MSL)/parseFloat(data[0].xnaaud) )*1000000000/1000000000).toFixed(10)     )
    {
   ...
   const res=await fetch("/buysellroute",{
     method:"POST",
     headers:{
       "Content-Type":"application/json"
     },
     body:JSON.stringify({
      description,MemBuyInAud,MemSellInXna
     })
   });
   const data=await res.json();
   if(res.status === 500 || !data) {  window.alert("Selling Not Done"); }
   else{ window.alert("Selling Done");
      }
    }
    }
  ...
  useEffect(() => {
    ...
    setData(json.al); const response3 = await fetch('/statement');
    const json3 = await response3.json();
    setSummary(json3.stat);
    const response4 = await fetch('/buysellSetget');
    const json4 = await response4.json();
    setDatabs(json4.bs2);
    const response5 = await fetch('/memberinfo');
...
if ( !data.length || !summary.length || !databs.length ) {
  ...
  return (
    <div className="xnarates">
        <h6 className="xnarateh6" style={{backgroundColor:"rgb(8, 143, 114)"}}>{t('Sell XNA')}</h6>
    ...
    <FormControl
      ...
      type="number" name="sellxna" value={sellxna} onChange={handleInputs} 
      required
    />
    <Button style={{backgroundColor:"rgb(8, 143, 114)",border:"none"}} id="button-addon2" onClick={postData}>
      {t('SELL')}
    </Button><br/>
    
  </InputGroup>
  <Form.Label htmlFor="basic-url">= {t('Equivalent Sell Amount In AUD')}</Form.Label>
  <InputGroup  className="mb-3">
  <FormControl
      ...
      aria-describedby="basic-addon2"
      disabled
      type="number" name="sellaud" value={sellaud} 
    />
  </InputGroup>
  {prefCurValue == 1 || prefCurValue == 0? 
   sellaud == 0 || sellaud == null || sellxna == 0 || sellxna == null? <PrefCurrSellRates sellamt={0}/> :
