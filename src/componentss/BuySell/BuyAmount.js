...
export default function BuyAmount() {
  ...
  const postData=async (e)=>{
    e.preventDefault();
    if (parseFloat(buyaud) > ( ( parseFloat(totdeposit)-parseFloat(totwithdraw) ) * (parseFloat(data[0].xnaaud) ) * (parseFloat(databs[0].BLP)/100)).toFixed(2))
    {
      console.log(" INPUT BUY MAX LIMIT ",parseFloat(buyaud))
      console.log("BUY MAX LIMIT ",( ( parseFloat(totdeposit)-parseFloat(totwithdraw) ) * (parseFloat(data[0].xnaaud) ) * (parseFloat(databs[0].BLP)/100)).toFixed(2))
      alert("Buy Limit exceeded")
      alert(( ( parseFloat(totdeposit)-parseFloat(totwithdraw) ) * (parseFloat(data[0].xnaaud) ) * (parseFloat(databs[0].BLP)/100)).toFixed(2))
   ...
   if(res.status === 500 || !data) {  window.alert("Buying Not Done"); }
   else{ window.alert("Buying Done");
      }
    }
    }
...
if ( !data.length || !summary.length || !databs.length ) {
  ...
  return (
    <div className="xnarates">
        <h6 className="xnarateh6" style={{backgroundColor:"rgb(8, 143, 114)"}}>{t('Buy XNA')}</h6>
        <p className="xnaratep" >{t('Buy Amount Details')}</p>
          <Table responsive style={{fontSize:"small"}}>
      <tbody>
        <tr>
            <td>{t('Maximum Buy Amount')}</td>
            <td>$ {( ( parseFloat(totdeposit)-parseFloat(totwithdraw) ) * (parseFloat(data[0].xnaaud) ) * (parseFloat(databs[0].BLP)/100)).toFixed(2)}</td>
        </tr>
        <tr>
    ...
    <FormControl
      ...
      type="number" name="buyaud" value={buyaud} onChange={handleInputs} 
    />
    <Button style={{backgroundColor:"rgb(8, 143, 114)",border:"none"}} id="button-addon2"  type="submit" onClick={postData}>
    {t('BUY')}
    </Button><br/>
    
  </InputGroup>
  <Form.Label htmlFor="basic-url">= {t('Equivalent Buy Amount In XNA')}</Form.Label>
  <InputGroup  className="mb-3">
  <FormControl
      ...
      aria-describedby="basic-addon2"
      disabled
      type="number" name="buyxna" value={buyxna}
    />
  </InputGroup>
  {prefCurValue == 1 || prefCurValue == 0?
  buyaud == 0 || buyaud == null? <PrefCurrBuyRates buyamt={0} />:
